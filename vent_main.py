...
def main(context):
    ...
    print('\nTraining Experiements Log Output Folder: %s' %output_path)
    ...
    if args.evaluate: 
        ...
        test_pred_path = '../test_pred/{}/{}'.format(train_exp, exp)
        mask_path = '../masks/{}/{}'.format(train_exp, exp)
    ...
    if args.parameters is not None and not args.evaluate:
        ...
        LOG.info('\n{0}'.format(parameters))
        print('\nParameters file: %s' %args.parameters)
        print('\n%s' %parameters)
    ...
    #########################################################################
    if args.encoder:
        model_type = args.arch + "_" + args.encoder
    else:
    ...
    if args.evaluate:
        print("\nEvaluation checkpoint '{}'".format(args.resume))
        ...
        dice, target, pred, outnames, volumes = validate(eval_loader, model, validation_log, global_step, args.start_epoch, save_pred=True, mask_path=mask_path)
...
def tversky_loss(y_pred, y_true):
    ...
    num = torch.sum(p0*g0, (0,1,2))
    den = num + alpha*torch.sum(p0*g1,(0,1,2)) + beta*torch.sum(p1*g0,(0,1,2))
    
    T = torch.sum(num/den) # when summing over classes, T has dynamic range [0 Ncl]
    
    Ncl = torch.cast(y_true.shape[-1], 'float32')
    return Ncl-T
...
def train(train_loader, model, optimizer, epoch, log):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        #LOG.info(i, np.array(target_var.data).max(), 'target max' )
        #LOG.info(i, np.array(target_var.data).min() )
        #LOG.info(i, np.array(input_var.data).max() , 'input max')
        #LOG.info(i, np.array(input_var.data).min() )
        ...
        #loss = tversky_loss(model_out, target_var)
...
def validate(eval_loader, model, log, global_step, epoch, save_pred=False, mask_path=None):
    ...
    volumes = {}
    subjs = {}
    outnames = pd.read_csv(args.test_slices, header=None)
    for i, (input, target) in enumerate(eval_loader):
        ...
        if save_pred:
            ...
            for sample in range(pred_var.size(0)):
                #LOG.info(outnames.size, sample, index, outnames)
                ...
                if subjname not in subjs: 
                    slicenum = 0
                    subjs[subjname] = slicenum
                    volumes[subjname] = [float(pred.sum()), float(target.sum())]
                else:
                    slicenum = subjs[subjname] + 1
                    subjs[subjname] = slicenum
                    volumes[subjname] = [ volumes[subjname][0] + float(pred.sum()), volumes[subjname][1] + float(target.sum())]

                #numbering = str(subjs[subjname]).zfill(3)
                #outpath = mask_path + "/" + subjname + "-" + numbering + ".jpg"
                outpath = mask_path + "/" + outname  + ".jpg"
                #LOG.info(outpath)
    ...
    if save_pred: 
        ...
        for subj, vols in volumes.items():
            pred = vols[0]
            target = vols[1]
            diff = round(abs(pred - target) / target * 100, 3)
            print('[%s]   pred: %s    target: %s  (%s)' %(subj, pred, target, diff))
     
        return overall_dice.item(), target_var.detach(), pred_var.detach(), outnames, volumes
    else: 
        LOG.info('Overall Validation Dice score: {}'.format(round(overall_dice.item(), 4) ))
        print('Overall Validation Dice score: {}\n'.format(round(overall_dice.item(), 4) ))
